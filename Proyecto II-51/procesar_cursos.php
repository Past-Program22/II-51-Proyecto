...
if (isset($accion) && $accion === 'eliminar' && isset($_GET['nombre_curso'])) {
  $id = $_GET['nombre_curso']; // Obtenemos el nombre del curso a eliminar desde la URL
  $sql = "DELETE FROM cursos WHERE nombre_curso = :nombre_curso"; // Consulta para eliminar el curso
  echo "<p>Eliminando curso con nombre: $nombre_curso</p>"; // Mensaje de depuración
  $stmt = $pdo->prepare($sql);
  $stmt->execute([':nombre_curso' => $nombre_curso]); // Ejecutamos la consulta con el curso proporcionado
  header('Location: cursos.php'); // Redirigimos al usuario a la página principal después de eliminar el registro
  exit;
}
